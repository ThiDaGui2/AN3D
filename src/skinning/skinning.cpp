...
namespace cgp
{
		...
		rig_structure const& rig)                     // information of the skinning weights (joints and weights associated to a vertex): check the header file to understand the rig structure
	{
		...
		// For each vertex, compute the skinned position and normal
		for(size_t i =0; i < N_vertex; ++i){
			vec3 p = vec3{0,0,0};
			vec3 n = vec3{0,0,0};
			// For each joint associated to the current vertex i
			for(size_t j =0; j < rig.joint[i].size(); ++j){
				int joint_index = rig.joint[i][j];
				float weight = rig.weight[i][j];
				affine_rt T = skeleton_current[joint_index] * inverse(skeleton_rest_pose[joint_index]);
				p += weight * (T * position_rest_pose[i]);
				n += weight * (T.rotation * normal_rest_pose[i]);
			}
			position_skinned[i] = p;
			normal_skinned[i] = normalize(n);
		}
